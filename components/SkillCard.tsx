  ...
  return (
    <div data-skill-card className="group relative bg-slate-900/40 backdrop-blur border border-slate-800 rounded-2xl p-5 hover:border-orange-500/50 transition-all hover:shadow-xl hover:shadow-orange-500/10">
      <div className="flex items-start gap-3 mb-3">
        <div className={`w-11 h-11 bg-gradient-to-br ${sourceColors[skill.source as keyof typeof sourceColors]} rounded-xl flex items-center justify-center text-xl flex-shrink-0 group-hover:scale-110 transition-transform`}>
          {skill.icon}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <span className={`inline-block px-2 py-0.5 rounded text-xs border ${sourceBadgeColors[skill.source as keyof typeof sourceBadgeColors]}`}>
            {skill.source}
          </span>
        </div>
      </div>
